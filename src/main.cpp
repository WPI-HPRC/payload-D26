...
void radioLoop() {
  ...
  if (millis() - lastRadioSendTime >= 200) {
    ...
    auto packetInner = hprc::CreatePayloadTelemetryPacket(builder, &sharedData, stateToTelemState(ctx.currentState), sensorBuilder.Finish());
    auto packet = hprc::CreatePacket(builder, hprc::PacketUnion_PayloadTelemetryPacket, packetInner.Union());
...
void sensorLoop() {
  ...
  */
  ...
  if (OUTPUT_SENSOR_DATA && millis() - last_print > 200) {
