...
StateID payloadROVLoop(StateData *data, Context *ctx) {
    ...
    if (ENABLE_OPENMV_RAW_MONITOR) {
        ...
        // drain any completed vision detection frame (blobs + horizon) parsed by
        // runReceiver and hand it to the antenna connector for the ground link
        VisionDetections detections;
        if (openMVReceiver.getDetections(detections)) {
            antennaConnector.setVisionDetections(detections);
        }
