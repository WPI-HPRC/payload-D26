...
#include <Arduino.h>
#include "../ImageTransfers/VisionDetections.h"
...
class AntennaConnectorInterface {
  public:
    ...
    /**
     * Latest vision detection frame (blobs + horizon) received from the camera.
     * Transmitter-side code reads this when building telemetry packets.
     */
    VisionDetections visionData;
    ...
    /**
     * Store the latest camera detection frame for transmission to the ground
     * station. Replaces any previously stored frame.
     */
    void setVisionDetections(const VisionDetections& detections);
