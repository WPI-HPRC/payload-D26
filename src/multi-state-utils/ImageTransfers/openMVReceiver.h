...
class OpenMVReceiver {
    public:
        ...
        /**
         * Returns true when a full detection frame (blobs + horizon) has been
         * received since the last getDetections() call.
         */
        bool hasDetections() const;

        /**
         * Copies the most recently completed detection frame into out and clears
         * the pending flag. Returns false when no new frame is available.
         */
        bool getDetections(VisionDetections& out);
    ...
    private:
        ...
        bool checkForDetectionStart(const String& receivedData);
        bool checkForDetectionEnd(const String& receivedData);
        void handleDetectionStart(const String& receivedData);
        void handleDetectionLine(const String& receivedData);
        void handleDetectionEnd();
        bool parseBlobLine(const String& receivedData, VisionBlob& blob);
        bool parseHorizonLine(const String& receivedData, VisionHorizon& horizon);
        ...
        bool parsingDetections = false;
        VisionDetections detectionFrame;     // frame currently being assembled
        VisionDetections latestDetections;    // last completed frame
